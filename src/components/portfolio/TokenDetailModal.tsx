...
export function TokenDetailModal({ isOpen, onClose, token }: TokenDetailModalProps) {
  ...
  const [swapTarget, setSwapTarget] = useState<PortfolioTokenBalance | null>(null);
  ...
  const toggleSwap = (tb: PortfolioTokenBalance) => {
    if (swapTarget && swapTarget.chainId === tb.chainId && swapTarget.token.address === tb.token.address) {
      setSwapTarget(null);
    } else {
      setSwapTarget(tb);
    }
  ...
  const renderTokenCard = (tb: PortfolioTokenBalance, isPrimary: boolean) => {
    ...
    const isSwapOpen = swapTarget && swapTarget.chainId === tb.chainId && swapTarget.token.address === tb.token.address;

    return (
      <div key={`${tb.chainId}-${tb.token.address}`} className={cn("p-2.5 rounded-xl space-y-1.5", isPrimary ? "bg-primary/5 border border-primary/20" : "bg-muted/20 border border-border/30")}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <ChainIcon chainId={tb.chainId} size="sm" className="w-3.5 h-3.5" />
            <Badge variant="outline" className="text-[9px] h-4 px-1">{cName}</Badge>
            {isPrimary && <Badge className="text-[9px] h-4 px-1 bg-primary/20 text-primary border-0">Current</Badge>}
          </div>
          <div className="text-right">
            <div className="text-xs font-semibold">{tb.balanceFormatted} {tb.token.symbol}</div>
        ...
        <div className="grid grid-cols-3 gap-1">
          <Button size="sm" variant={isSwapOpen ? "default" : "outline"} className="text-[10px] h-6 gap-0.5 px-1.5" onClick={() => toggleSwap(tb)}>
            <ArrowRightLeft className="w-3 h-3" /> Swap
          </Button>
          <Button size="sm" variant="outline" className="text-[10px] h-6 gap-0.5 px-1.5" onClick={() => handleEarn(tb)}>
            <TrendingUp className="w-3 h-3" /> Earn
          </Button>
          <Button size="sm" variant="outline" className="text-[10px] h-6 gap-0.5 px-1.5" onClick={() => handleVaults(tb)}>
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div
              className="w-full max-w-md bg-card border border-border rounded-2xl shadow-xl overflow-hidden max-h-[85vh] overflow-y-auto"
                  ...
                  <AnimatePresence>
                    {showOtherChains && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, height: 'auto' }}
                        exit={{ opacity: 0, height: 0 }}
                        className="overflow-hidden space-y-1.5 max-h-[200px] overflow-y-auto"
                      >
